...
function EntityEditor({
    ...
    return (
        <div className="bg-white rounded-lg sm:rounded-xl border border-gray-200 shadow-sm p-4 sm:p-6">
            <p className="text-xs sm:text-sm font-semibold text-gray-900 mb-3 sm:mb-4">Manage {config.pluralLabel}</p>
            ...
            {state.loading ? (
                ...
                <p className="text-xs sm:text-sm text-gray-500 text-center py-3 sm:py-4">No {config.pluralLabel.toLowerCase()} yet. Add one above!</p>
            ) : (
                <div className="space-y-1">
                    {state.list.map((item) => (
                        <div key={item.id} className="flex items-center gap-2 py-1.5 sm:py-2 px-2 sm:px-3 rounded-lg hover:bg-gray-50 group">
                            {state.editingId === item.id ? (
                                <>
                                    ...
                                    />
                                    <button onClick={() => onUpdate(item.id)} className="text-green-600 hover:text-green-700 flex-shrink-0">
                                    ...
                                    <button onClick={onCancelEdit} className="text-gray-400 hover:text-gray-600 flex-shrink-0">
                                ...
                                <>
                                    <span className="flex-1 text-xs sm:text-sm text-gray-900 truncate">{config.displayName(item)}</span>
                                    <button onClick={() => onStartEdit(item)} className="opacity-0 group-hover:opacity-100 text-blue-600 hover:text-blue-700 transition-opacity flex-shrink-0">
                                    ...
                                    <button onClick={() => onDelete(item.id)} className="opacity-0 group-hover:opacity-100 text-red-600 hover:text-red-700 transition-opacity flex-shrink-0">
...
export default function AdminDashboard() {
    ...
    const [demoteConfirmDialog, setDemoteConfirmDialog] = useState<{ show: boolean; personId: string | null; personName: string }>({ show: false, personId: null, personName: '' });
    ...
    const fetchStats = async () => {
        if (!userEmail) return;
        try {
            const response = await fetch(`/api/admin/dashboard?email=${encodeURIComponent(userEmail)}`);
    ...
    const confirmDemoteAdmin = (person: OrgPerson) => {
        setDemoteConfirmDialog({ show: true, personId: person.person_id, personName: getPersonDisplayName(person) });
    };
    ...
    const handleDemoteAdmin = async () => {
        ...
        try {
            const response = await fetch(`/api/admin/admins?email=${encodeURIComponent(userEmail)}&person_id=${personId}`, {
                method: 'DELETE',
            });
            const data = await response.json();
                ...
                <div className="max-w-7xl mx-auto px-3 sm:px-6 py-3 sm:py-4 flex items-center justify-between">
                    <div className="flex items-center gap-2 sm:gap-3">
                        ...
                        <div className="min-w-0">
                            <p className="text-xs sm:text-sm font-bold text-gray-900 truncate">{stats?.organizationName || 'SAC Skill Tracker'}</p>
                            <p className="text-[10px] sm:text-xs text-gray-500 hidden sm:block">Administrator Dashboard</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-2 sm:gap-3">
            ...
            <main className="max-w-7xl mx-auto px-3 sm:px-6 py-4 sm:py-8 space-y-4 sm:space-y-6">
                ...
                {loading && (
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 sm:p-4 flex items-center gap-2 sm:gap-3">
                        ...
                        <p className="text-xs sm:text-sm text-blue-800">Loading dashboard statistics...</p>
                    </div>
                )}
                ...
                {activeTab === 'roster' && (
                    <div className="bg-white rounded-lg sm:rounded-xl border border-gray-200 shadow-sm">
                        <div className="p-4 sm:p-6 flex items-center justify-between border-b border-gray-100">
                            <h2 className="text-sm sm:text-base font-semibold text-gray-900">Import from SportsEngine</h2>
                        </div>
                        ...
                        <div className="p-4 sm:p-6">
                            <div className="border-2 border-dashed border-gray-200 rounded-lg sm:rounded-xl p-6 sm:p-10 flex flex-col items-center text-center mb-4 sm:mb-6">
                                <svg className="w-10 h-10 sm:w-12 sm:h-12 text-gray-300 mb-2 sm:mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><ellipse cx="12" cy="12" rx="9" ry="4" strokeWidth={1.5} /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 12c0 2.21 4.03 4 9 4s9-1.79 9-4M3 16c0 2.21 4.03 4 9 4s9-1.79 9-4" /></svg>
                                <p className="text-sm sm:text-base font-semibold text-gray-900 mb-1">Import Roster Data</p>
                                <p className="text-xs sm:text-sm text-gray-500 mb-4 sm:mb-5">Import swimmer roster, levels, and class assignments from SportsEngine</p>
                            ...
                            <div>
                                <p className="text-xs sm:text-sm font-semibold text-gray-900 mb-2">Supported Import Options:</p>
                ...
                {activeTab === 'admins' && (
                    <div className="bg-white rounded-lg sm:rounded-xl border border-gray-200 shadow-sm p-4 sm:p-6">
                        <h2 className="text-sm sm:text-base font-semibold text-gray-900 mb-3 sm:mb-4">Organization Admins</h2>

                        <div className="mb-4 sm:mb-6">
                            <p className="text-xs sm:text-sm font-medium text-gray-800 mb-2">Promote instructor to admin</p>
                        ...
                        <div>
                            <p className="text-xs sm:text-sm font-medium text-gray-800 mb-2">Current admins</p>
                            {adminsLoading ? (
                                ...
                                <p className="text-xs sm:text-sm text-gray-500">No admins found.</p>
                            ) : (
                                <div className="space-y-1">
                                    {admins.map((person) => (
                                        <div key={person.person_id} className="px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-lg border border-gray-100 bg-gray-50 flex items-center justify-between gap-2">
                                            <div className="flex-1 min-w-0">
                                                <p className="text-xs sm:text-sm text-gray-900 truncate">{getPersonDisplayName(person)}</p>
                                                {person.email && <p className="text-[10px] sm:text-xs text-gray-500 truncate">{person.email}</p>}
                                            </div>
                                            <button
                        ...
                        {demoteConfirmDialog.show && (
                            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                                <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
                                    <h3 className="text-lg font-semibold text-gray-900 mb-2">Demote Admin</h3>
                                    <p className="text-sm text-gray-600 mb-4">
                                        Are you sure you want to demote <strong>{demoteConfirmDialog.personName}</strong> from admin? They will keep their instructor permissions but lose admin access.
                                    </p>
                                    <div className="flex gap-3 justify-end">
                                        <button
                                            onClick={() => setDemoteConfirmDialog({ show: false, personId: null, personName: '' })}
                ...
                {activeTab !== 'roster' && activeTab !== 'admins' && activeTab !== 'instructors' && activeTab !== 'classes' && (
                    <EntityEditor
